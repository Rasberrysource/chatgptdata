...
> 구현 결과와 실행 방법은 저장소 루트의 `README.md` 참고.
...
방법(실제 적용):
1. 위키백과 부문별 목록 10개 페이지를 **리비전 고정** 스냅샷으로 저장하고 27개 시즌 전 부문을 구조화했다(각 표는 공식 보도자료·주요 매체를 각주로 인용).
2. 같은 위키백과의 회차별 문서 216개와 수상작·후보 목록을 자동 대조했다 — 270개 부문×시즌 중 242개 일치, 27개는 문서 형식 차이로 대조 불가, 1개는 대조 도구 오탐(수동 확인 결과 충돌 없음).
3. 공식 사이트 직접 대조는 범위를 제한했다. oscars.org·bafta.org는 이 환경에서 403으로 막혔고, 나머지 공식 사이트의 추가 교차검증은 “실제 충돌이 발견된 항목만 추가 검증”한다는 방침에 따라 진행하지 않았다. 대신 위키백과가 각 시상식 행에 인용한 공식 URL을 레코드마다 기록해 사이트에서 바로 확인할 수 있게 했다.
4. 모든 레코드는 사용한 리비전 permalink와 검증 상태를 가진다.
